 ...
 * the JPA datastore.
 * 
 * @see com.carpool.entities.UserActivity
...
public class UserActivityDAO implements IUserActivityDAO {
	...
	public void save(UserActivity entity) {
		EntityManagerHelper.log("saving UserActivity instance", Level.INFO,
				null);
		try {
	...
	public void delete(UserActivity entity) {
		EntityManagerHelper.log("deleting UserActivity instance", Level.INFO,
				null);
		try {
			entity = getEntityManager().getReference(UserActivity.class,
					entity.getId());
	 ...
	 * <pre>
	 * EntityManagerHelper.beginTransaction();
	 * entity = UserActivityDAO.update(entity);
	 * EntityManagerHelper.commit();
	 * </pre>
	 * 
	 * @param entity
	...
	public UserActivity update(UserActivity entity) {
		EntityManagerHelper.log("updating UserActivity instance", Level.INFO,
				null);
		try {
			UserActivity result = getEntityManager().merge(entity);
	...
	public UserActivity findById(UserActivityId id) {
		EntityManagerHelper.log("finding UserActivity instance with id: " + id,
				Level.INFO, null);
		try {
			UserActivity instance = getEntityManager().find(UserActivity.class,
					id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List<UserActivity> findByProperty(String propertyName,
			final Object value, final int... rowStartIdxAndCount) {
		EntityManagerHelper.log("finding UserActivity instance with property: "
				+ propertyName + ", value: " + value, Level.INFO, null);
		try {
			final String queryString = "select model from UserActivity model where model."
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<UserActivity> findAll(final int... rowStartIdxAndCount) {
		EntityManagerHelper.log("finding all UserActivity instances",
				Level.INFO, null);
		try {
			final String queryString = "select model from UserActivity model";
			Query query = getEntityManager().createQuery(queryString);
			if (rowStartIdxAndCount != null && rowStartIdxAndCount.length > 0) {
				int rowStartIdx = Math.max(0, rowStartIdxAndCount[0]);
				if (rowStartIdx > 0) {
